...
public class MemoryUploading implements Uploading<DataBuffer> {
  private final AllSpaces<DataBuffer> allSpaces;

  public MemoryUploading(AllSpaces<DataBuffer> allSpaces) {
    this.allSpaces = allSpaces;
  }

  @Override
  public CompletionStage<Object<DataBuffer>> upload(String spaceName, Publisher<Payload<DataBuffer>> contents) {
    return start()
      .thenCompose(__ -> allSpaces.belongingTo(spaceName))
      .thenCompose(space -> objects(space, contents));
  }

  private static CompletionStage<Object<DataBuffer>> objects(Space<DataBuffer> space, Publisher<Payload<DataBuffer>> contents) {
